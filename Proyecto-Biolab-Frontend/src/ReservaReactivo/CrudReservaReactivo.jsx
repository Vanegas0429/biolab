...
const CrudReservaReactivo = () => {
  ...
  const groupedData = useMemo(() => {
    ...
    ReservaReactivo.forEach(item => {
      ...
      groups[resId].reactivosList.push({
        nombre: item.Reactivo?.Nom_reactivo || `Reactivo ${item.Id_Reactivo}`,
        cantidad: item.Can_Reactivo
      });
    });
  ...
  const getAllReservaReactivo = async () => {
    try {
        const response = await apiAxios.get('/api/ReservaReactivo')
        setReservaReactivo(response.data)
    } catch (error) {
        console.error("Error cargando Reserva-Reactivo:", error)
    }
  }
  ...
  return (
    <>
      <div className="container-fluid px-4 px-md-5 mt-5" style={{ paddingTop: '2rem' }}>
        <h2 className="mb-4 fw-bold text-dark"><i className="fa-solid fa-flask-vial me-2 text-primary"></i>Reactivos por Reserva</h2>
        
        <div className="row mb-4">
          <div className="col-12 col-md-4">
            <input 
              className="form-control shadow-sm" 
              placeholder="Buscar por reserva, solicitante o reactivo..." 
              value={filterText} 
            ...
            <div className="modal-content border-0 shadow">
              ...
              <div className="modal-body p-0">
                <div className="list-group list-group-flush">
                  {selectedReserva?.reactivosList.map((r, index) => (
                    <div key={index} className="list-group-item d-flex justify-content-between align-items-center p-3">
                      ...
                      <span className="badge bg-secondary rounded-pill px-3">Cant: {r.cantidad}</span>
                    </div>
                  ))}
                </div>
              </div>
